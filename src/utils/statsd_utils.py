...
class RetryingStatsClient(StatsClient):
    ...
    def __init__(self, host: str = 'localhost', port: int = 8125, prefix: Optional[str] = None,
    ...
    def _send(self, data: str) -> None:
        ...
        if self._sock is None and time.time() - self._last_attempt > _delay_seconds_between_connection_retries:
            try:
                fam = socket.AF_INET6 if ipv6 else socket.AF_INET
                family, _, _, _, addr = socket.getaddrinfo(self.host, self.port, fam, socket.SOCK_DGRAM)[0]
                self._addr = addr
                self._sock = socket.socket(family, socket.SOCK_DGRAM)
                self._prefix = prefix
                log.info("Successfully connected to StatsD host %s:%s", self.host, str(self.port))
            except Exception as e:
                log.exception("Failed to connect to StatsD host %s:%s : %s", self.host, self.port, e)
...
statsd: RetryingStatsClient = RetryingStatsClient(host=host, port=port, prefix=prefix, maxudpsize=maxudpsize, ipv6=ipv6)
